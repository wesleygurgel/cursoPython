...
class Conta:
    ...
    def depositar(self: object, valor: float) -> None:
        if valor > 0:
            ...
            print(f'Deposito efetuado com sucesso no valor de {formata_float_str_moeda(valor)} na conta {self.numero}!')
        else:
    ...
    def transferir(self: object, valor: float, destinatario: object) -> None:
        if valor > 0 and self.saldo_total >= valor:
            ...
            print(f'Transferência de {self.numero} para {destinatario.numero} realizada com Sucesso!')
        else:
